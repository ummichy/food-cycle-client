...
const MyFoodRequest = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 sm:px-4 py-10 mt-24">
      {requests.length === 0 ? (
        <p className="text-center text-gray-500 italic">No requests found.</p>
      ) : (
        <div className="overflow-x-auto rounded-xl border border-gray-300 shadow-xl bg-[#edeceb]">
          <table className="min-w-full text-xs sm:text-sm rounded-xl overflow-hidden">
            <thead className="bg-[#cec9c5] sticky top-0 z-10">
              <tr>
                <th className="text-black px-2 sm:px-4 md:px-6 py-2 sm:py-3 text-left text-[10px] sm:text-xs font-semibold uppercase">#</th>
                <th className="text-black px-2 sm:px-4 md:px-6 py-2 sm:py-3 text-left text-[10px] sm:text-xs font-semibold uppercase">Food</th>
                <th className="text-black px-2 sm:px-4 md:px-6 py-2 sm:py-3 text-left text-[10px] sm:text-xs font-semibold uppercase">Donor</th>
                <th className="text-black px-2 sm:px-4 md:px-6 py-2 sm:py-3 text-left text-[10px] sm:text-xs font-semibold uppercase">Pickup</th>
                <th className="text-black px-2 sm:px-4 md:px-6 py-2 sm:py-3 text-left text-[10px] sm:text-xs font-semibold uppercase">Expires</th>
                <th className="text-black px-2 sm:px-4 md:px-6 py-2 sm:py-3 text-left text-[10px] sm:text-xs font-semibold uppercase">Requested</th>
                <th className="text-black px-2 sm:px-4 md:px-6 py-2 sm:py-3 text-center text-[10px] sm:text-xs font-semibold uppercase">Actions</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {requests.map((req, index) => (
                <tr key={req._id} className="hover:bg-gray-50 transition">
                  <td className="px-2 sm:px-4 md:px-6 py-2 sm:py-4 text-[10px] sm:text-sm font-medium text-gray-700">{index + 1}</td>
                  <td className="px-2 sm:px-4 md:px-6 py-2 sm:py-4">
                    <div className="flex items-center gap-2 sm:gap-3">
                      ...
                      />
                      <div>
                        <p className="font-medium text-gray-800 text-[10px] sm:text-sm">{req.foodName}</p>
                        <p className="text-gray-400 text-[8px] sm:text-xs">ID: {req.foodId?.slice(-5)}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-2 sm:px-4 md:px-6 py-2 sm:py-4 text-[10px] sm:text-sm text-gray-700">{req.donorName}</td>
                  <td className="px-2 sm:px-4 md:px-6 py-2 sm:py-4 text-[10px] sm:text-sm text-gray-700">{req.pickupLocation}</td>
                  <td className="px-2 sm:px-4 md:px-6 py-2 sm:py-4 text-[10px] sm:text-sm text-gray-700">
                    {new Date(req.expireDate).toLocaleString(undefined, {
                      dateStyle: 'medium',
                      timeStyle: 'short',
                    })}
                  ...
                  <td className="px-2 sm:px-4 md:px-6 py-2 sm:py-4 text-center">
                    <button
                      onClick={() => cancelRequest(req._id)}
                      className="px-2 py-1 sm:px-4 sm:py-2 bg-black text-white rounded-md hover:bg-gray-800 transition text-[10px] sm:text-sm"
                    >
                      Cancel
                    </button>
                  </td>
